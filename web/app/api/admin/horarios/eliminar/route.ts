...
import { eliminarHorario } from "@/lib/repos/horarios";
...
export async function POST(req: Request) {
  ...
  const ok = await eliminarHorario(id);
  if (ok) {
    await auditLog({
    ...
    return NextResponse.json({ success: true, message: "Bloque horario eliminado." });
  }
  return NextResponse.json({ success: false, message: "Error al eliminar el bloque." });
}
