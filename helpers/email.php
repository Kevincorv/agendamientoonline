<?php
function enviarConfirmacionCita(array $cita): bool {
    if (empty($cita['email'])) return false;

    $cancelUrl = APP_URL . '/cancelar-cita?token=' . $cita['token_cancelacion'];
    $asunto = "Confirmación de cita - " . APP_NAME;
    $mensaje = "
    <html><body style='font-family: Arial, sans-serif; color: #333;'>
        <div style='max-width:600px;margin:auto;padding:20px;border:1px solid #e5e7eb;border-radius:8px;'>
            <h2 style='color:#1e40af;'>✅ Cita Agendada - " . APP_NAME . "</h2>
    ...
    return mail($cita['email'], $asunto, $mensaje, $headers);
}
